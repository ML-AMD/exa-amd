...
def vasp_calculations(config):
    ...
    output_file_vasp_calc = os.path.join(config["vasp_work_dir"],config["output_file"])
    ...
    exec_time = -time.time()
    l_futures = [run_vasp_calc(config.get_json_config(),i) for i in range(config["nstart"], config["nend"])]
