    ...
    if len(structures_data) < min_structures:
        ...
        for i, (index, ef) in enumerate(all_structures):
            if i >= min_structures:
                break
            structures_data[index] = ef
        actual_ef_threshold = all_structures[min_structures-1][1] if len(all_structures) >= min_structures else all_structures[-1][1]
        print(f"Adjusted Ef threshold to {actual_ef_threshold:.3f} to ensure minimum of {min_structures} structures")
    else:
        print(f"Selected {len(structures_data)} structures with original Ef threshold {ef_threshold}")
...
def process_structures(task_queue, result_queue, nomix_dir, natom_threshold, element_fractions):
    while True:
        ...
        if task is None:
            break
        index, ef = task
        structure = Structure.from_file(os.path.join(nomix_dir, f"{index}.cif"))
...
def select_structures(nomix_dir, output_dir, csv_file, ef_threshold, min_total, max_total, num_workers, natom_threshold, element_fractions):
    ...
    for _ in range(num_workers):
        p = mp.Process(target=process_structures, args=(task_queue, result_queue, nomix_dir, natom_threshold, element_fractions))
        p.start()
        processes.append(p)
    ...
    processes = []
    for _ in range(num_workers):
        p = mp.Process(target=select_structures_for_compositions, args=(task_queue, result_queue, matcher))
        p.start()
        processes.append(p)

    # Add tasks to the queue
    for composition in sorted_compositions:
        task_queue.put((composition, composition_groups[composition], n_per_composition))
    ...
    if len(selected_structures) < min_total:
        print(f"Warning: The final number of selected structures ({len(selected_structures)}) is less than the specified minimum ({min_total}).")
        print("This may be due to a lack of sufficiently diverse structures in the dataset or the filtering criteria.")

def main():
    parser = argparse.ArgumentParser(description="Optimized Multiprocessing Structure Selection Script")
    parser.add_argument("--nomix_dir", default="nomix/", help="Directory containing hypo structures")
    parser.add_argument("--output_dir", default="new/", help="Output directory for selected structures")
    parser.add_argument("--csv_file", default="test_results.csv", help="Path to test_results.csv")
    parser.add_argument("--ef_threshold", type=float, default=-0.2, help="Ef threshold for selection")
    parser.add_argument("--min_total", type=int, default=1000, help="Minimum total number of structures to select")
    parser.add_argument("--max_total", type=int, default=4000, help="Maximum total number of structures to select")
    parser.add_argument("--num_workers", type=int, default=mp.cpu_count(), help="Number of worker processes to use")
    parser.add_argument("--natom_threshold", type=int, default=50, help="Maximum number of atoms in reduced formula")
    parser.add_argument("--element_fractions", type=str, default="", help="Minimum atomic fractions for elements, format: 'Element1:fraction1,Element2:fraction2,...'")

    args = parser.parse_args()

    # Parse element fractions
    element_fractions = {elem: float(frac) for elem, frac in [pair.split(':') for pair in args.element_fractions.split(',') if pair]}
